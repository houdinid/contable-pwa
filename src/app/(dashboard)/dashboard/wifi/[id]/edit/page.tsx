...
import { WifiDeviceType, WifiNetwork } from "@/types";

export default function EditWifiPage() {
    const router = useRouter();
    const params = useParams();
    const { updateWifiNetwork, wifiNetworks, contacts } = useData();
    ...
    useEffect(() => {
        if (wifiNetworks.length > 0 && params.id) {
            const network = wifiNetworks.find(w => w.id === params.id);
            if (network) {
                setSsid(network.ssid);
                setPassword(network.password || "");
                setEncryption(network.encryption || "WPA2");
                setIsHidden(network.isHidden || false);
                setDeviceType(network.deviceType);
                setDeviceBrand(network.deviceBrand || "");
                setModel(network.model || "");
                setClientId(network.clientId || "");
                setArea(network.area || "");
                setIpAddress(network.ipAddress || "");
                setSubnetMask(network.subnetMask || "");
                setGateway(network.gateway || "");
                setDns(network.dns || "");
                setNotes(network.notes || "");
                setPreviewUrl(network.photoUrl || null);
                setIsLoading(false);
            } else {
                router.push("/dashboard/wifi");
            }
        } else if (!isLoading && wifiNetworks.length === 0) {
            // If persisted data takes a moment or is empty
            const timer = setTimeout(() => setIsLoading(false), 1000);
            return () => clearTimeout(timer);
        }
    }, [params.id, wifiNetworks, router, isLoading]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await updateWifiNetwork(params.id as string, {
                ssid,
                password,
                encryption,
                isHidden,
            ...
            console.error(error);
            alert("Error al actualizar la red WiFi");
        } finally {
    ...
    if (isLoading) {
        return <div className="p-8 text-center text-gray-500">Cargando información de la red...</div>;
    }
    ...
    return (
        <div className="max-w-4xl mx-auto pb-10">
            ...
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-foreground">Editar Red WiFi</h1>
                    <p className="text-gray-500 dark:text-gray-400">Actualizar configuración para {ssid || "..."}</p>
                </div>
            </div>
