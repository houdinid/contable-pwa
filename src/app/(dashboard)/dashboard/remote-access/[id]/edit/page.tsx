...
export default function EditRemoteAccessPage() {
    const router = useRouter();
    const params = useParams();
    const id = params?.id as string;

    const { remoteAccesses, updateRemoteAccess, contacts } = useData();
    ...
    useEffect(() => {
        if (!id || remoteAccesses.length === 0) return;

        const access = remoteAccesses.find(a => a.id === id);
        if (access) {
            setClientId(access.clientId || "");
            setSoftwareType(access.softwareType || "AnyDesk");
            setHostname(access.hostname || "");
            setConnectionCode(access.connectionCode || "");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            alert("Error al actualizar el acceso remoto");
        } finally {
    ...
    return (
        <div className="max-w-4xl mx-auto pb-10">
            ...
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-foreground">Editar Acceso Remoto</h1>
                    <p className="text-gray-500 dark:text-gray-400">Actualiza las credenciales o detalles asignados.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-foreground mb-1">Contraseña (Mantenla en blanco para no cambiarla)</label>
                            <input
                                type="text"
                                value={password}
